...
function usage() {
    ...
    echo "  cp-1    run setup script for cp-1"
    echo "  cp-2    run setup script for cp-2"
    echo "  cp-3    run setup script for cp-3"
    echo "  wk-*    run setup script for wk-*"
}

case $1 in
    cp-1|cp-2|cp-3|wk-*)
        echo "testyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy"
        ;;
    help)
        usage
        exit 255
...
helm install argocd argo/argo-cd \
    ...
    --create-namespace \
    --namespace argocd \
    --values https://raw.githubusercontent.com/KanameImaichi/fivem-project/"${TARGET_BRANCH}"/k8s-manifests/argocd-helm-chart-values.yaml
helm install argocd argo/argocd-apps \
    --version 0.0.1 \
    --values https://raw.githubusercontent.com/KanameImaichi/fivem-project/"${TARGET_BRANCH}"/k8s-manifests/argocd-apps-helm-chart-values.yaml
