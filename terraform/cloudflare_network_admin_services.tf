...
resource "cloudflare_access_application" "onp_argocd" {
  zone_id          = local.cloudflare_zone_id
  name             = "Argocd administration"
  domain           = "argocd.onp-k8s.admin${local.root_domain}"
  type             = "self_hosted"
...
resource "cloudflare_access_policy" "onp_argocd" {
  application_id = cloudflare_access_application.onp_argocd.id
  zone_id        = local.cloudflare_zone_id
